...
public class ProtoEditor extends ViewPart implements PropertyChangeListener {
			...
			private void updateBar() {
				PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell()
						.getDisplay().asyncExec(new Runnable() {
							...
							public void run() {
								int height = 0;
								for (ExpandItem nestedItem : innerBar
										.getItems()) {
									height += nestedItem.getControl()
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if (evt.getPropertyName().equals("PROTO_CHANGE")) {
			...
			FieldDescriptorContainer field = (FieldDescriptorContainer) evt
					.getOldValue();
			Composite comp = field.getParent().getParent();
			if (comp != null) {
				currentBar = (ExpandBar) comp;
				FieldDescriptorContainer added = (FieldDescriptorContainer) evt
						.getNewValue();
